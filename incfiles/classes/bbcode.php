 ...
 * @copyright   Copyright (C) 2005-2016 VinaJohnCMS Community
...
class bbcode extends core
{
    ...
    */
    ...
    {
        $var = preg_replace_callback('#@([\w\d]{2,})#si', 'Tagf', $var); //tag thanh vien by TOMDZ
        $var = self::youtube($var);
        $var = self::parse_time($var);               // Обработка тэга времени
        $var = self::highlight_bb($var);             // Обработка ссылок
        $var = self::highlight_code($var);            // Обработка ссылок
...
///BBCODE by TOMDZ - Vina4u.Pro ///

private static function youtube($var)
    {
            ...
            function ($matches) {
               ...
               global $is_mobile;
                if(preg_match('/youtube.com/', $matches[1])){
                   ...
                   if (!$is_mobile)
                       return '<div><iframe width="560" height="315" src="http://www.youtube.com/embed/'. $valuesto[0] . '?feature=player_detailpage" frameborder="0" allowfullscreen></iframe></div>';
                   else
                       return '<div><center><a target="_blank" href="http://m.youtube.com/watch?v=' . $valuesto[0] . '"><img src="http://img.youtube.com/vi/' . $valuesto[0] . '/1.jpg" border="0" alt="youtube.com/embed/' . $valuesto[0] . '" frameborder="0" allowfullscreen></a></center></div>';
                } else {
    ...
    */
    private static function highlight_bb($var)
    {
        ...
        $replace = array(
            ...
            '<img src="$1" border="0" style="max-width:80%;"></img><br />', //images 
            '<i class="fa fa-cloud-download"></i><a href="$1" title="Click to download">[Download]</a>',// link download
