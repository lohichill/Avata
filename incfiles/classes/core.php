...
class core
{
    ...
    private $flood_interval = '9999'; // Интервал времени в секундах
    private $flood_limit = '500'; // Число разрешенных запросов за интервал
    ...
    function __construct()
    {
        ...
        $this->gzip_lib_class(); // Aгружаем язык
    ...
    private function ip_flood()
    {
        if ($this->flood_chk) {
            ...
            if ($requests > $this->flood_limit) {
                die('FLOOD: Tải lại đã vượt quá giới hạn yêu cầu cho phép');
            }
        }
    }
    ...
    */
    ...
    private function gzip_lib_class()
    {
        require(ROOTPATH . 'incfiles/lib/gzip.lib.php');
        ...
        if (mysql_num_rows($req)) {
            ...
            switch ($res['ban_type']) {
                case 2:
                    ...
                    else header('Location: http://vina4u.biz');
                    exit;
                    break;
                case 3:
