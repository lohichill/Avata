...
if ($dem > $kmess){echo '<div class="phantrang">' . functions::display_pagination2('more.php?', $start, $dem, $kmess) . '</div>';
}
} else {
...
case 'nhom':
echo '<div class="mainblok"><div class="phdr"><b>Danh sách nhóm</b></div>';
...
while($res=mysql_fetch_array($req)) {
$nhom = nhom($res['id']);
echo '<div class="omenu"><img src="'.$home.'/images/clan/'.$nhom['icon'].'.png"> <a href="page.php?id='.$res['id'].'"><b><font color="2c5170">'.$nhom['name'].'</font></b></a><br/ >'.catchu($nhom['gt'],0,10).' ...</div>';
}
if ($dem > $kmess){echo '<div class="phantrang">' . functions::display_pagination2('more.php?act=nhom&', $start, $dem, $kmess) . '</div>';
}
} else {
...
$nhom = nhom($id);
echo '<div class="mainblok"><div class="phdr"><b>Thành viên</b></div>';
...
while($res=mysql_fetch_array($req)) {
echo '<div class="omenu">'.ten_nick($res['user_id'],1,$res['id']).'<br/>'.($res['user_id']!=$user_id && $nhom['user_id']==$user_id ? '<a href="set.php?id='.$id.'&sid='.$res['user_id'].'"><b><font color="2c5170">Chức vụ</font></b></a> - <a href="set.php?act=duoi&id='.$id.'&sid='.$res['user_id'].'" style="color:red;"><b>Đuổi</b></a>':'').'</div>';
}
if ($tong > $kmess){echo '<div class="phantrang">' . functions::display_pagination2('more.php?act=mem&id='.$id.'&', $start, $tong, $kmess) . '</div>';
}
echo '<div class="omenu"><a href="page.php?id='.$id.'">Trở về nhóm >></a></div></div>';
break;
case 'like':
...
if ($tong > $kmess){echo '<div class="topmenu">' . functions::display_pagination2('more.php?act=like&id='.$id.'&', $start, $tong, $kmess) . '</div>';
}
echo '</div>';
